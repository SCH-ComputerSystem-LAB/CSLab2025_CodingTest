...
Baekjoon #2531 회전 초밥
- 알고리즘: 투 포인터, 슬라이딩 윈도우
    - 1차원 배열에서 각자 다른 원소를 가리키는 2개의 포인터를 조작해가면서 원하는 값을 찾을 때까지 탐색하는 알고리즘
    - 슬라이딩 윈도우에 좀 더 가까운듯,,
...
- 오답노트
    - 헷갈리는 점
        - 연속해서 먹는 접시의 개수는 최대 k개인가 k개 고정인가... > 일단 고정 기준으로 풀기
"""
...
max = 0
eatif = deque()
for tmp in (list[-(k-1):]+list[:]):
    if len(eatif) >= k: eatif.popleft()
    eatif.append(tmp)
    vn = len(set(eatif)) if eatif.count(c)!=0 else len(set(eatif))+1
    if max < vn: max = vn
print(max)